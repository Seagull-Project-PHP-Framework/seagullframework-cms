...
class Menu_ExplorerBsd
{
    ...
    function render($id = 0)
    {
        ...
        $this->translateItems($menu->menu->items);
        $html = $menu->printMenu();
        return $html;
    }

    function translateItems($aItems)
    {
        foreach ($aItems as $key => $oItem) {
            if (isset($oItem->items) && is_array($oItem->items)) {
                $this->translateItems($oItem->items);
            }
            // translate text node
            if (!empty($oItem) && $oItem instanceOf HTML_TreeNode) {
                $oItem->text = SGL_String::translate($oItem->text);
            }
        }
    }
    ...
    function getGuruTree($id = 0)
    {
        ...
        require_once SGL_MOD_DIR . '/cms/classes/HTML_TreeMenu_DHTML_SGL.php';
        $theme = isset($this->options['theme'])
            ? $this->options['theme']
            : 'default';
