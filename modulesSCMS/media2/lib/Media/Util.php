...
class SGL_Media_Util
{
    ...
    public static function deleteMediaImage($fileName, $container = 'default')
    {
        $confFile = SGL_Config::locateCachedFile(SGL_MOD_DIR . '/media2/image.ini');

        // make sure container name is valid
        $container = strtolower(str_replace(' ', '_', $container));

        $oImage = new SGL_Image($fileName);
        $oImage->init($confFile, $container);
        $oImage->delete();
    }
}
?>