...
class MediaUploaderMgr extends SGL_Manager
{
        ...
        if ($input->action == 'upload') {
            ...
            if (!empty($input->oMedia) && empty($input->oMedia->error)) {
                ...
                if ($mimeType) {
                    ...
                    SGL::raiseError('not allowed content type', null, null, true);
                }
            }
        } else {
            $valid = true;
    ...
    public function _cmd_upload(SGL_Registry $input, SGL_Output $output)
    {
        ...
        $oMime     = $this->da->getMimeInfoByMimeType($input->oMedia->mime_type);
        $fileName .= '.' . $oMime->extension;
        ...
        $aFields['media_mime_id'] = $oMime->media_mime_id;
        ...
        if (!PEAR::isError($mediaId)) {
            ...
            $output->oResponse->mediaName  = $input->oMedia->name;
        }
        ...
        // upload regular media
            ...
            if (!PEAR::isError($ok)) {
                ...
                {
                    SGL::raiseError('can not move uploaded file', null, null, true);
                }
            }
        }
    }
